...
echo "[*]	Converting 02.HTTPROBE.ALIVE.lst to 03.HTTPROBE.ALIVE.IPs.lst"
	...
	sudo massdns -r ~/WORKZONE/tools/massdns/lists/resolvers.txt -t A -o S -w massdns-results.txt 02.HTTPROBE.ALIVE.lst
...
echo "[*]	Probing IPs & Ports via 05_NMAP-SCAN.sh : Versioning, banners, vulners, http-vulners-regex, http-headers"
	...
	mkdir -p ~/WORKZONE/rawdata/$HOST/nmap-data/xml	# searchsploit --nmap *.xml
...
echo "[*]	Running: 06_EYEWITNESS.sh on 02.HTTPROBE.ALIVE.lst --web" 
	...
	python3 ~/WORKZONE/tools/EyeWitness/Python/EyeWitness.py --prepend-https -f 02.HTTPROBE.ALIVE.lst -d $PWD/eyewitness --no-prompt --timeout 120 --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36"
	rm geckodriver.log
...
echo "[*]	Running: 07_ANALYZE.sh on $HOST Directory structures" 
	...
	find ~/WORKZONE/rawdata/$HOST -type d -print0 | xargs -0 sudo chown -R user:user
