...
echo "[*]	Probing IPs & Ports via 05_NMAP-SCAN.sh : Versioning, banners, vulners, http-vulners-regex, http-headers"
	...
	mkdir -p $HOME/WORKZONE/rawdata/$HOST/nmap-data/xml		# searchsploit --nmap *.xml
...
echo "[*]	Running: 06_GOWITNESS.sh on 02.HTTPROBE.ALIVE.lst" 

	python3 $HOME/WORKZONE/tools/EyeWitness/Python/EyeWitness.py --prepend-https -f 02.HTTPROBE.ALIVE.lst -d $PWD/eyewitness --no-prompt --timeout 120 --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36"
	rm geckodriver.log
